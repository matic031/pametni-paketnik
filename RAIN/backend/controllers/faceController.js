...
const faceController = {
    ...
    /**
     * Verify face with base64 image (for mobile app)
     */
    verifyFaceBase64: async (req, res) => {
        try {
            console.log('Face verification (base64) request received');

            const { user_id, base64_image } = req.body;

            if (!user_id || !base64_image) {
                return res.status(400).json({
                    success: false,
            ...
            console.log(`Sending face verification (base64) to ${FACE_API_URL}/verify_base64 for user ${user_id}`);

            const response = await axios.post(`${FACE_API_URL}/verify_base64`, {
                user_id,
                base64_image
            }, {
                headers: {
            ...
            if (response.data.success) {
                ...
                if (verified) {
                    ...
                    console.log(`Face verification (base64) successful for user ${user_id} (score: ${similarity_score})`);
                    ...
                    console.log(`Face verification (base64) failed for user ${user_id} (score: ${similarity_score})`);
            ...
            console.error('Face verification (base64) error:', error.message);
